...
export class LeagueTableComponent {
  ...
  getPositionIcon(index: number): { icon: string, class: string, title: string } | null {
    const divName = this.division().name;
    const country = this.countryId();
    const compId = this.competitionId();
    ...
    // 2. National League Continental Qualification
    if (country && (divName.includes('Primeira') || divName.includes('Única') || divName.includes('Conferência') || divName === 'Série A')) {
      if (this.isEuropean()) {
        if (['ESP', 'ENG', 'ITA', 'GER', 'FRA'].includes(country)) {
          if (index < 4) return { icon: 'fa-angle-right', class: 'text-blue-400', title: 'Vaga Direta Champions League' };
          if (index < 8) return { icon: 'fa-angle-right', class: 'text-orange-400', title: 'Vaga Direta Europa League' };
        } else if (country === 'POR') {
          if (index < 4) return { icon: 'fa-angle-right', class: 'text-blue-400', title: 'Vaga Direta Champions League' };
          if (index < 7) return { icon: 'fa-angle-right', class: 'text-orange-400', title: 'Vaga Direta Europa League' };
        } else if (country === 'BEL') {
          if (index < 2) return { icon: 'fa-angle-right', class: 'text-blue-400', title: 'Vaga Direta Champions League' };
          if (index < 5) return { icon: 'fa-angle-right', class: 'text-orange-400', title: 'Vaga Direta Europa League' };
        } else if (['RUS', 'NED', 'TUR'].includes(country)) {
          if (index < 2) return { icon: 'fa-angle-right', class: 'text-blue-400', title: 'Vaga Direta Champions League' };
          if (index < 4) return { icon: 'fa-angle-right', class: 'text-orange-400', title: 'Vaga Direta Europa League' };
        }
      } else if (this.isSouthAmerican()) {
        const spots = this.samSpots[country];
        if (spots) {
          if (index < spots.lib) return { icon: 'fa-angle-right', class: 'text-green-400', title: 'Vaga para Libertadores' };
          if (index < spots.lib + spots.lib_po) return { icon: 'fa-angles-right', class: 'text-orange-400', title: 'Vaga para Pré-Libertadores' };
          if (index < spots.lib + spots.lib_po + spots.sul) return { icon: 'fa-angle-right', class: 'text-blue-400', title: 'Vaga para Sulamericana' };
        }
      } else if (country === 'USA') {
        if (index < 8) return { icon: 'fa-trophy', class: 'text-yellow-500', title: 'Vaga MLS Cup' };
      } else if (this.isNorthAmerican()) {
        if (index < 8) return { icon: 'fa-angle-right', class: 'text-green-400', title: 'Vaga Direta America Champions League' };
      } else if (this.isAfrican()) {
        if (this.paisesVagaDireta.includes(country)) {
        ...
        // Asia e outros
        if (index < this.qualificationSpots[country]) {
          return { icon: 'fa-angle-right', class: 'text-green-400', title: 'Vaga Asia Champions League' };
        }
      }
    }
